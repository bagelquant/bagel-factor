...
class TestQuantileReturns(unittest.TestCase):
    def setUp(self):
        ...
        self.futuren_returns = pd.Series(np.tile(np.arange(10, 70, 10), 3), index=idx, name='returns')
        ...
        self.returns_nan = self.futuren_returns.copy()
    ...
    def test_quantile_returns_custom_labels(self):
        qrets = quantile_returns(self.factor, self.futuren_returns, n_quantiles=2, quantile_labels=['low', 'high'])
        self.assertListEqual(list(qrets.columns), ['low', 'high'])

    def test_quantile_returns_constant(self):
        qrets = quantile_returns(self.factor_const, self.futuren_returns, n_quantiles=3)
    ...
    def test_index_mismatch(self):
        with self.assertRaises(ValueError):
            quantile_returns(self.factor, self.futuren_returns.drop(self.futuren_returns.index[0]))
