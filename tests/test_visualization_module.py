...
def test_plot_helpers_smoke() -> None:
    ...
    qr = pd.DataFrame(
        {1: [0.01, 0.02, -0.01, 0.00, 0.03], 2: [0.02, 0.01, 0.00, 0.01, 0.04]}, index=idx
    )
...
def test_plot_result_summary_smoke() -> None:
    ...
    result = MockResult(
        ...
        ic={
            1: pd.Series(
                [0.1, -0.2, 0.05, 0.0, 0.3, 0.15, -0.1, 0.2, 0.1, 0.05], index=idx, name="ic"
            ),
            5: pd.Series(
                [0.2, -0.1, 0.1, 0.05, 0.25, 0.1, -0.05, 0.15, 0.12, 0.08], index=idx, name="ic"
            ),
        },
        quantile_returns={
            1: pd.DataFrame(
        ...
        long_short={
            1: pd.Series(
                [0.01, -0.02, 0.01, 0.0, 0.02, 0.01, -0.01, 0.02, 0.01, 0.00],
                index=idx,
                name="long_short",
            ),
            5: pd.Series(
                [0.02, -0.01, 0.02, 0.01, 0.03, 0.02, -0.02, 0.03, 0.02, 0.01],
                index=idx,
                name="long_short",
            ),
        },
